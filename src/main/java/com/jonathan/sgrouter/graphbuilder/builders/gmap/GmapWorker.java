...
public class GmapWorker {
	List<String> srcList = new ArrayList<>(), desList = new ArrayList<>(), servList = new ArrayList<>();
	...
	public GmapTiming getAvgTiming(TransitMode mode) {
		...
		for (int i = 0; i < srcList.size(); i++) {
			GmapTiming t = gmapConn.getGmapSpeed(srcList.get(i), desList.get(i), mode, servList.get(i), defaultTiming);
