...
public class ShpParser {
	public static List<ShpNode> parse(String filename, boolean trainExit) { // Filename excludes extension
		List<ShpNode> output = new ArrayList<>();
		try {
			CoordinateReferenceSystem crs = CRS
					.parseWKT(new String(Files.readAllBytes(Paths.get(filename + ".prj")), StandardCharsets.UTF_8));
			...
			while (features.hasNext()) {
				...
				if (ids.isBlank()) {
					if (GraphBuilderApplication.config.graphbuilder.train.getNameToIds()
							.containsKey(name.toLowerCase()))
						ids = GraphBuilderApplication.config.graphbuilder.train.getNameToIds().get(name.toLowerCase());
					else {
						System.out.println(String.format("Unknown station: %s", name));
						continue;
					}
				}
				...
				if (trainExit) {
					...
					if (exitStr == null || exitStr.equals("NULL"))
						exitStr = " EXIT";
					else
						exitStr = " EXIT " + exitStr;
					output.add(new ShpNode(ids, name + exitStr, result.y, result.x));
				} else
					for (int i = 0; i < splitId.length; i++)
						output.add(new ShpNode(splitId[i], name, result.y, result.x));

				// System.out.println(String.format("%s: %s | %s",feature.getAttribute("STN_NO"),feature.getAttribute("STN_NAME"),feature.getAttribute("the_geom")));
			}
			features.close();
			ds.dispose();
		} catch (Exception e) {
